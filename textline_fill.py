...
def estimate_text_to_fit(hindi_text, bbox_width_inches, bbox_height_inches, box_id, output_json_path, x1, y1, font_size, bboxes):
    # Find the current box using the provided box_id
    current_box = next((bbox for bbox in bboxes if bbox[5] == box_id), None)
    if current_box:
        # Extract image_id and label from the current bounding box
        image_id = current_box[6]  # Get the image ID from bbox[6]
        label = current_box[4]     # Get the label from bbox[4]

        # Construct the JSON file path based on image_id
        json_folder = r'output_jsons'
        output_json_path = os.path.join(json_folder, f'{image_id}.json')
        ...
        if max_lines > 0 and '\n'.join(truncated_text_lines):
            # Find all annotations that match the box_id
            annotations = [ann for ann in output_json["annotations"] if ann["id"] == box_id]
            if annotations:
                for annotation in annotations:
                    # Initialize textlines if not already present
                    if "textlines" not in annotation:
                        annotation["textlines"] = []

                    # Append new text lines to the existing textlines
                    for idx, line in enumerate(truncated_text_lines):
                        annotation['textlines'].append({
                            "line_idx": len(annotation["textlines"]),  # Index for the new line
                            "bbox": [x1, y1 + len(annotation["textlines"]) * line_height, bbox_width_inches * 72, line_height],
                            "text": line
                        })
                        print(line)

                    combined_text = '\n'.join([line['text'] for line in annotation['textlines']])
                    
                    # Write combined text to the "text" field of the annotation
        ...
        print(f"No bounding box found with box_id: {box_id}")
        return
...
def get_bboxes_and_image_path(base_path):
    ...
    for subdir, _, files in os.walk(bbox_dir):
        ...
        for bbox_file in bbox_files:
            ...
            # Extract the image name by removing the last part after the last underscore
            # For example: "0301_0000_0008_846" becomes "0301_0000_0008"
    ...
    for bbox in bboxes:
        ...
        if label not in ["figure_1", "formula", "formula_1", "QR code", "table", "page number", "figure", "page_number", "mugshot", "code", "correlation", "bracket", "examinee info", "sealing line", "weather forecast", "barcode", "bill", "advertisement", "underscore", "blank", "other question number", "second-level question number", "third-level question number", "first-level question number"]:
            ...
            if label.lower() in ["headline","author","credit","index"] and height > width:
                ...
                estimated_lines = hindi_text_to_fit.split('\\linebreak')
